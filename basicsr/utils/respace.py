...
from basicsr.models.gaussian_diffusion import GaussianDiffusion, GaussianDiffusionSR
...
class SpacedDiffusionSR(GaussianDiffusionSR):
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        self.timestep_map = []
        self.original_num_steps = len(kwargs["sqrt_etas"])
        base_diffusion =GaussianDiffusionSR(**kwargs) # pylint: disable=missing-kwoa
        new_sqrt_etas = []
        for ii, etas_current in enumerate(base_diffusion.sqrt_etas):
            if ii in self.use_timesteps:
                new_sqrt_etas.append(etas_current)
