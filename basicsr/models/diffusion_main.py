...
class Unet(nn.Module):
    ...
    def forward(self, x, time,lq, x_self_cond=None):
        ...
        for block1, block2, attn, upsample in self.ups:
            h_=h.pop()
            x = F.interpolate(x, size=h_.shape[2:], mode='bilinear',
                                      align_corners=True)
...
class UnetRes(nn.Module):
    ...
    def forward(self, x, time, lq,x_self_cond=None):
        if self.share_encoder == 1:
            ...
            return [self.unet0(x, time, lq,x_self_cond=x_self_cond)]
...
class ResidualDiffusion(nn.Module):
    ...
    def p_losses(self, gt_,lq_,t, noise=None):
        x_input=lq_
        x_start=gt_
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def forward(self, gt_,lq_,t):
        return self.p_losses(gt_,lq_,t)